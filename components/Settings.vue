<template>
  <div v-click-away="close" class="fixed top-0 left-0 mt-2 ml-4 text-gray-700 dark:text-gray-300">
    ...
    <div class="settings-wrapper ml-1 flex flex-col font-light text-sm p-4 shadow-th rounded-xl overflow-hidden bg-white dark:bg-gray-800" :class="{ open }">
      ...
      <div class="mb-4">
        <button title="Light" class="display-button" :class="{ 'bg-gray-200 dark:bg-gray-600': $store.state.darkTheme === false }" @click="$store.commit('theme', 'light')"><i class="fas fa-sun"></i></button>
        <button title="Dark" class="display-button ml-1" :class="{ 'bg-gray-200 dark:bg-gray-600': $store.state.darkTheme === true }" @click="$store.commit('theme', 'dark')"><i class="fas fa-moon"></i></button>
        <button title="Follow system preference" class="display-button ml-1" :class="{ 'bg-gray-200 dark:bg-gray-600': $store.state.darkTheme === null }" @click="$store.commit('darkTemeAuto')"><i class="fas fa-magic"></i></button>
      </div>
    </div>
  </div>
</template>
