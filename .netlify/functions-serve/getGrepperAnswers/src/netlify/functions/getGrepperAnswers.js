...
// netlify/functions/getGrepperAnswers.js
var import_axios = __toModule(require_axios2());
exports.handler = async ({ queryStringParameters }, context) => {
  var _a, _b;
  const query = queryStringParameters.query;
  if (!query)
    return { statusCode: 400, body: "Query parameter not provided" };
  try {
    const response = await import_axios.default.get(`https://www.codegrepper.com/api/get_answers_1.php?s=${query}`);
    if (response.data.answers && response.data.answers.length) {
      ...
      const googleRes = await import_axios.default.get(`https://www.googleapis.com/customsearch/v1?cx=${process.env.GOOGLE_SEARCH_CX}&q=${encodeURI(query)}&key=${process.env.GOOGLE_SEARCH_KEY}`);
      const googleResults = (_b = (_a = googleRes.data) == null ? void 0 : _a.items) == null ? void 0 : _b.map((result) => result.link);
      if (googleResults == null ? void 0 : googleResults.length) {
        const response2 = await import_axios.default.post("https://www.codegrepper.com/api/get_answers_2.php", {
 ...
 * @license  MIT
 */
//# sourceMappingURL=getGrepperAnswers.js.map
